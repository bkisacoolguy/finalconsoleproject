  ...
  return (
    <div className=' border-1 border-black min-h-[300px]'>
      <div>
        ...
        <p className='text-xl mb-[0.5rem]'>Initialise Connection</p>
...
SariskaMediaTransport.initialize()`}
      ...
      <button disabled={stepno==1} onClick={()=>stepupdateminus(2)} className={`border-1 py-1 px-4 ${
    stepno === 1
      ? 'border-gray-400 text-gray-400 cursor-not-allowed'
      : 'border-black text-black'
  }`}
 >Back</button>
      <button disabled={stepno==5} onClick={()=>stepupdateplus(4)} className={`border-1 py-1 px-4 text-blue-500 border-blue-300 ${
    stepno === 6
      ? 'border-gray-400 text-gray-400 cursor-not-allowed'
      : 'border-black text-black'
  }`}>Next</button>
